...
签到（issue #72）不是 `club_activities` 上的一个子状态，而是独立的 `club_activity_check_ins` 表——一个活动可以有任意多条签到记录，互不阻塞取消/结项等仍未实现的能力（见下）。两种签到方式：
...
> 旧版文档还设想过"取消活动""活动结束后补发布""活动结束后提交图文材料再审核"等能力，这几项在当前代码里仍然没有对应的接口或状态字段——`club_activities` 除了签到之外，仍只有一次性的"创建 → （可多次）修改"两个动作。`picture_urls` 字段存在（活动图片列表），但只能通过"修改活动"申请整体替换，没有独立的"活动信息提交并二次审核"流程。
