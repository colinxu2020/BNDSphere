...
| `ix_club_members_club_id` | 条件唯一索引 | `club_id` WHERE `membership = 'president'`：一个社团至多一位社长 |
...
部署工作流会在**旧应用仍在提供服务时**运行新版本的迁移（见 `infra/deploy/lib/stack.sh` 的 `run_update`：`run_migration` 排在 `recreate_services` 之前）。在迁移提交完成、而容器尚未被替换的这段时间里，应用 N-1 正运行在 schema N 上。
...
版本 N 的迁移绝不能立即移除版本 N-1 所依赖的 schema。这一点无法自动强制执行——部署工作流无法推断意图——因此必须在代码评审时进行检查。
...
（本节此前声称"Alembic 会单独提交每一个 revision"，据此推导出一个需要 N-1 策略兜底的"部分升级窗口"。该前提是错误的，那个窗口并不存在。N-1 策略依然是必需的，但理由是下面这一个。）

### N-1 策略的依据：迁移先于替换

迁移刻意排在容器替换之前——迁移失败必须在任何东西被替换之前中止。代价是存在一个窗口：迁移已经提交，而 `compose up` 尚未完成，此时应用 N-1 正在 schema N 上读写。这个窗口只有几秒，但它是真实的，也是上面那张 expand/migrate/contract 表格存在的原因。

还有一个长得多的窗口。这个工作流**没有自动回滚**（见 `stack.sh` 顶部的 `NO ROLLBACK`）：一次失败的部署把坏版本留在原地并如实报告，恢复由人来做——把上一个版本的容器放回去，而 schema 停在 N。这段时间不由代码决定，取决于人什么时候到场。

这条规则无法自动强制执行：没有任何工具能推断一次迁移的意图，因此它只能靠上面的评审清单在 code review 时把住。这是本设计里唯一一条依赖人的不变量，必须如实记录为此。

### 为什么不用 `alembic downgrade`

回退到上一个版本时不运行任何反向迁移。这不是图省事：

- **`downgrade()` 换不回数据。** `DROP COLUMN` 的反向操作是 `ADD COLUMN`——列回来了，里面是空的。用 downgrade 去「撤销」一次破坏性迁移，等于把一次可恢复的回滚换成一次静默的数据丢失。而 N-1 兼容性反过来让回滚**不需要**碰 schema。
- **`downgrade()` 从不被执行，因此从未被验证。** CI 和生产都只跑 `upgrade`。自动生成的 downgrade 分支第一次真正运行，就是在故障现场。
- **失败的部署里 schema 往往是好的。** 最常见的失败是新容器没能变健康，迁移早已成功提交。此时 downgrade 是去破坏一个正在正常工作的 schema。
- **迁移失败的情形不需要它。** 整个 `upgrade head` 在一个事务里（见「多 revision 升级的失败语义」），失败即整体回滚，schema 原地不动，部署在替换任何东西之前中止。

所以「靠人工把删除推迟一个版本」不是 downgrade 的退让方案，它就是这条策略本身：**删除晚一个版本，回滚就永远只是换容器。**

### N+2 → N+1 为什么不会因为缺列而失败

SQLAlchemy 的 ORM 不发 `SELECT *`：语句按 mapper 上**声明**的列展开，`INSERT` 同理。所以两种不匹配并不对称：

- 库里**多**出模型没有声明的列——无害，它不进任何语句。
- 模型声明了库里**没有**的列——必然失败。

N+2 删掉的那一列，按定义在 N+1 就已经不再被读取、也不再被声明（这正是「N+1 只从新结构读取」的含义）。因此 N+2 回滚到 N+1 落在第一种情况。会落到第二种情况的是**跨版本**回滚（N+2 直接退到 N），或者 contract 提前到了 N+1——这也正是删除必须等到 N+2、且回滚一次只退一个版本的原因。

写入方向由清单第 4 项守住：N 新增的 `NOT NULL` 列必须有默认值或已回填，否则 N-1 省略该列的 `INSERT` 会失败。
