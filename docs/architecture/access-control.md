...
`GET /users/me` 返回 `CurrentUserInfo.effective_roles`，由同一能力表派生，
列出当前身份能满足的非封禁角色要求。RootLayout 依据该字段显示管理员、
社联、审核入口；字段缺失时隐藏这些入口，不在前端重新推导继承关系。
公开用户资料和管理员用户列表不增加该字段。后端检查始终是授权依据；
能力信息只是读取时的快照，角色变更后的实际访问仍由服务器检查。
