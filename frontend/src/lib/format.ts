...
const ERROR_CODE_MESSAGES: Record<string, string> = {
  ...
  CLUB_MEMBER_NOT_FOUND: "没有找到这个社团成员",
  CLUB_PRESIDENT_REQUIRED: "只有社长可以管理成员",
  ...
  CANNOT_REMOVE_PRESIDENT: "社长不能将自己移出社团，请先交接社团",
  RECORD_NOT_FOUND: "没有找到这条申请记录",
  RECORD_REVIEWED: "这条活动记录已经审核过，不能再次修改",
  RESOURCE_FILE_ALREADY_REGISTERED: "该文件已经登记到资料中心",
  RESOURCE_FILE_NOT_FOUND: "没有找到这个资料文件",
...
const MESSAGE_KEY_TEXT: Record<string, string> = {
  ...
  "error.club.member_not_found": "没有找到这个社团成员",
  "error.club.president_required": "只有社长可以管理成员",
  "error.club.cannot_change_president_role": "社长不能直接变更自己的职位，请先交接社团",
  "error.club.cannot_remove_president": "社长不能将自己移出社团，请先交接社团",
  ...
  "error.star_level.denied": "已审核的星级评价申请不能再次审核",
...
function formatBackendError(value: unknown): string {
  ...
  const payload =
    record.response && typeof record.response === "object"
      ? ((record.response as Record<string, unknown>).data as Record<string, unknown>) || record
      : record;
...
function formatValidationMessage(record: Record<string, unknown>): string {
  ...
  const ctx =
    record.ctx && typeof record.ctx === "object" ? (record.ctx as Record<string, unknown>) : {};
  ...
  if (msg.includes("String should have at most")) {
    return typeof ctx.max_length === "number" ? `不能超过 ${ctx.max_length} 个字符` : "长度过长";
  }
  if (msg.includes("String should have at least")) {
    return typeof ctx.min_length === "number" ? `至少需要 ${ctx.min_length} 个字符` : "长度过短";
  }
