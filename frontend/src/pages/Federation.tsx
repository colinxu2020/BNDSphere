        ...
        >
          <Field label="活动 ID">
            <input
              ...
              onChange={(event) => setEditActivityId(event.target.value)}
              required
            />
          </Field>
          <Field label="活动层级">
        ...
        {isLoading ? (
          <div className="grid gap-4">
            {[...Array(3)].map((_, index) => (
              <div
                key={index}
                className="animate-pulse bg-slate-50 h-36 rounded-2xl border border-slate-100"
              />
            ))}
          </div>
        ) : activities.length ? (
                ...
                <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4">
                  <div>
                    <div className="flex flex-wrap gap-2">
                      ...
                      <Badge>
                        {activityItem.club_records?.length || 0} 条记录
                      </Badge>
                    </div>
                    ...
                    <p className="text-xs text-slate-400 font-medium mt-2">
                      #{activityItem.id} · {formatDate(activityItem.created_at)}
                    </p>
                  </div>
                  <div className="flex flex-wrap gap-2 shrink-0">
                    ...
                    <DangerButton
                      type="button"
                      onClick={() => deleteActivity(activityItem.id)}
                    >
                      <Trash2 size={16} /> 删除
                    </DangerButton>
                  </div>
