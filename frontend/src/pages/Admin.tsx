...
export function Admin() {
  ...
  const [actionTone, setActionTone] = useState<"error" | "success" | "info">(
    "error",
  );
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Surface>
          ...
          <form onSubmit={submitClub} className="flex flex-col gap-4">
            ...
            <SecondaryButton
              type="button"
              onClick={loadClubForAdmin}
              disabled={!clubId || isClubLoading}
            >
              <RefreshCw size={16} />{" "}
              {isClubLoading ? "读取中..." : "管理员读取社团"}
            </SecondaryButton>
            <Field label="简介">
              <input
                className={inputClassName}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <Field label="星级">
                <select
                  ...
                  value={clubStarLevel}
                  onChange={(event) =>
                    setClubStarLevel(event.target.value as ClubStarLevel | "")
                  }
                >
                  <option value="">不修改</option>
              ...
              <Field label="状态">
                <select
                  ...
                  value={clubStatus}
                  onChange={(event) =>
                    setClubStatus(event.target.value as ClubStatus | "")
                  }
                >
                  <option value="">不修改</option>
      ...
      <Surface>
        ...
        >
          ...
          <div className="md:col-span-2 flex flex-col sm:flex-row gap-3 sm:justify-end">
            <PrimaryButton type="submit" loading={isTermSubmitting}>
              创建学期
            </PrimaryButton>
            <SecondaryButton
              type="button"
              onClick={submitTermUpdate}
              disabled={isTermSubmitting || !termId}
            >
              更新学期
            </SecondaryButton>
          </div>
        ...
        {termsError && (
          <div className="mb-5">
            <StatusMessage value={termsError} />
          </div>
        )}
        ...
        {isTermsLoading ? (
          <div className="grid gap-4">
            {[...Array(3)].map((_, index) => (
              <div
                key={index}
                className="animate-pulse bg-slate-50 h-28 rounded-2xl border border-slate-100"
              />
            ))}
          </div>
        ) : terms.length ? (
                ...
                <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                  <div>
                    ...
                    <p className="text-sm text-slate-500 mt-1">
                      #{term.id} · {formatDate(term.start_date)} -{" "}
