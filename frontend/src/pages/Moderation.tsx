...
import { Check, Clock, FilePenLine, RefreshCw, X } from 'lucide-react';
...
type ModerationItem = UserRequest | ActivityCreateRequest | ActivityUpdateRequest | ClubUpdateRequest;
...
function renderRequestDetails(item: ModerationItem) {
  ...
  if ('start_time' in item) rows.push(['开始时间', item.start_time ? formatDateTime(item.start_time) : null]);
  if ('end_time' in item) rows.push(['结束时间', item.end_time ? formatDateTime(item.end_time) : null]);
...
export function Moderation() {
  ...
  const fetchQueue = async () => {
    ...
    try {
      ...
      if (activeQueue === 'activityCreate') {
        const { data, error } = await client.GET('/api/v1/moderations/club-activities/create-requests', {
          params: { query: { size: 50 } },
        });
        if (error) setError(error);
        setItems(data?.items || []);
      ...
      if (activeQueue === 'activityUpdate') {
        const { data, error } = await client.GET('/api/v1/moderations/club-activities/update-requests', {
          params: { query: { size: 50 } },
        });
        if (error) setError(error);
        setItems(data?.items || []);
  ...
  const moderate = async (requestId: number, moderationStatus: ModerationStatus) => {
    ...
    try {
      ...
      if (activeQueue === 'activityCreate') {
        result = await client.PATCH('/api/v1/moderations/club-activities/create-requests/{request_id}', {
          params: { path: { request_id: requestId } },
          body,
        });
      }
      ...
      if (activeQueue === 'activityUpdate') {
        result = await client.PATCH('/api/v1/moderations/club-activities/update-requests/{request_id}', {
          params: { path: { request_id: requestId } },
          body,
        });
      }
  ...
  return (
    ...
    >
      ...
      <Surface>
        ...
        {actionMessage && <div className="mb-5"><StatusMessage value={actionMessage} tone={actionTone} /></div>}
        {error && <div className="mb-5"><InlineError value={error} /></div>}

        {isLoading ? (
          <div className="grid gap-4">
            {[...Array(3)].map((_, index) => (
              <div key={index} className="animate-pulse bg-slate-50 h-40 rounded-2xl border border-slate-100" />
            ))}
          </div>
        ) : items.length ? (
          <div className="grid gap-4">
                ...
                <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4">
                  <div className="min-w-0">
                    <div className="flex flex-wrap items-center gap-2">
                      <Badge tone={item.moderation_status === 'approved' ? 'green' : item.moderation_status === 'rejected' ? 'red' : 'yellow'}>
          ...
          <EmptyState title="没有待处理请求" description={error ? stringifyBackendValue(error) : undefined} />
        )}
      </Surface>
    </motion.div>
  );
