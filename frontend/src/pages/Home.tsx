...
export function Home() {
  ...
  const boardItems = useMemo(
    () =>
      activities
        .filter((activity) => activity.poster_uri)
        .slice(0, 5),
    [activities],
  );
      ...
      <div className="grid gap-6 lg:grid-cols-[1.55fr_0.95fr]">
        <section
          className={cn(
            ...
            isLoggedIn ? "min-h-[250px]" : "min-h-[320px]",
          )}
        >
          {isLoading ? (
            <div className="h-full min-h-[250px] animate-pulse bg-slate-100" />
          ) : boardItems.length ? (
        ...
        <section className="rounded-md border border-slate-200 bg-white p-5">
          <div className="mb-4 flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <h2 className="font-display text-lg font-bold">活动日历</h2>
            </div>
          ...
          <div className="mt-2 grid grid-cols-7 gap-1">
            {calendar.days.map((day, index) => (
              <div
                ...
                className={cn(
                  "group relative flex aspect-square min-h-10 flex-col items-center justify-center rounded-sm border border-transparent text-sm",
...
function BoardPanel({ items }: { items: GeneralActivity[] }) {
  ...
  return (
    <a
      ...
      rel="noreferrer"
      className="group relative block h-full min-h-[250px] bg-slate-950"
    >
      {active.poster_uri ? (
        <img
          src={active.poster_uri}
          alt={active.name}
          className="h-full min-h-[250px] w-full object-fill"
        />
      ) : (
        <div className="flex h-full min-h-[250px] items-center justify-center bg-slate-800 text-slate-500">
          <Image size={42} />
        </div>
      )}
