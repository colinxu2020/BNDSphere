    ...
    >
      ...
      <section className="flex flex-col items-center text-center mt-12 md:mt-20 gap-6">
        ...
        <p className="text-lg md:text-xl text-slate-500 max-w-2xl text-balance">
          BNDSphere
          是你在十一学校探索、加入和管理社团的入口。用清晰的流程处理社团、活动与审核。
        </p>
      ...
      <section className="flex flex-col gap-6 mt-8">
        <div className="flex items-end justify-between">
          ...
          <Link
            to="/explore"
            className="hidden sm:flex text-sm font-medium text-primary-600 hover:text-primary-700 items-center gap-1 group transition-colors"
          >
            查看全部{" "}
            <ArrowUpRight
              size={16}
        ...
        {isLoading ? (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {[...Array(3)].map((_, index) => (
              <div
                key={index}
                className="animate-pulse bg-white h-52 rounded-3xl border border-slate-100"
              />
            ))}
          </div>
        ) : clubs.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {clubs.map((club, idx) => (
              ...
              >
                <Link
                  to={`/club/${club.id}`}
                  className="group relative flex flex-col bg-white p-6 rounded-3xl border border-slate-100 shadow-[0_4px_20px_-4px_rgba(0,0,0,0.05)] hover:shadow-[0_8px_30px_-4px_rgba(0,0,0,0.1)] transition-all duration-300 hover:border-primary-100 overflow-hidden text-left h-full"
                >
                  ...
                  <div className="flex items-center gap-4 mb-4">
                    ...
                    <div>
                      ...
                      <h3 className="font-semibold text-lg text-slate-900 leading-tight group-hover:text-primary-600 transition-colors">
                        {club.name}
                      </h3>
                    </div>
                  </div>
        ...
        <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-8 text-center md:text-left">
          <div className="max-w-xl">
            ...
            <p className="text-slate-400 text-lg">
              从发现社团到提交审核请求，把校园社团事务放在同一个清晰入口中处理。
            </p>
          </div>

          <div className="flex gap-8 md:gap-12">
            <div className="flex flex-col items-center md:items-start text-center md:text-left">
              <span className="flex items-center gap-2 text-primary-400 text-sm font-semibold uppercase tracking-wider mb-2">
                <Users size={16} /> 社团总数
              </span>
            ...
            <div className="flex flex-col items-center md:items-start text-center md:text-left">
              <span className="flex items-center gap-2 text-secondary-400 text-sm font-semibold uppercase tracking-wider mb-2">
                <Star size={16} /> 当前展示
              </span>
              <span className="text-4xl md:text-5xl font-display font-bold text-white">
                {clubs.length}
