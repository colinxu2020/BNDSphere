import { Sparkles, Calendar, MapPin, Users, Activity, Hash, ArrowLeft, Settings } from 'lucide-react';
...
export function ClubDetail() {
  ...
  if (isLoading) {
    return <div className="animate-pulse space-y-8 flex flex-col">
  ...
  return (
    ...
    >
      <Link to="/explore" className="inline-flex items-center gap-2 text-slate-500 hover:text-slate-900 font-medium w-fit transition-colors">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            {club.club_activities && club.club_activities.length > 0 ? (
              <div className="grid gap-4">
                {club.club_activities.map(act => (
                  <div key={act.id} className="bg-white border border-slate-200 p-6 rounded-[2rem] flex flex-col sm:flex-row gap-4 sm:items-center justify-between shadow-sm hover:shadow-md transition-shadow group">
                    <div className="flex flex-col gap-1">
                       <h3 className="font-semibold text-lg text-slate-900 group-hover:text-primary-600 transition-colors">{act.name}</h3>
                       ...
                       <div className="flex items-center gap-4 mt-2 text-xs font-medium text-slate-400">
                          <span className="flex items-center gap-1.5 bg-slate-50 px-2 py-1 rounded-md"><Calendar size={14}/> {new Date(act.start_time).toLocaleDateString()}</span>
                          <span className="flex items-center gap-1.5 bg-slate-50 px-2 py-1 rounded-md"><MapPin size={14}/> {act.location}</span>
                       </div>
                    </div>
        ...
        <div className="flex flex-col gap-6">
          <div className="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-sm flex flex-col gap-6">
             <h3 className="font-display font-bold text-lg text-slate-900">社团概览</h3>
             ...
             <div className="flex items-start gap-4">
                ...
                <div>
                   ...
                   <p className="text-lg font-bold text-slate-900">{club.members?.length || 0} 名成员</p>
                </div>
             </div>
             ...
             <div className="flex items-start gap-4">
                ...
                <div>
                   ...
                   <p className="text-lg font-bold text-slate-900">{club.club_activities?.length || 0} 场 / 学期</p>
                </div>
             </div>
             ...
             <div className="flex items-start gap-4">
                ...
                <div>
                   ...
                   <p className="text-lg font-bold text-slate-900 text-emerald-700">{STATUS_MAP[club.status] || club.status}</p>
                </div>
             </div>
          </div>
