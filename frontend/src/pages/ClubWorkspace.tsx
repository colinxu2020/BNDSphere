  ...
  const submitActivityUpdate = async (event: React.FormEvent) => {
    ...
    try {
      const { data, error } = await client.POST(
        ...
        {
          params: {
            path: { club_id: clubId, activity_id: Number(updateActivityId) },
          },
          body: {
            name: nullableText(updateActivityName),
            description: nullableText(updateActivityDescription),
            start_time: updateActivityStart
              ? fromDateTimeLocalValue(updateActivityStart)
              : null,
            end_time: updateActivityEnd
              ? fromDateTimeLocalValue(updateActivityEnd)
              : null,
            location: nullableText(updateActivityLocation),
            picture_urls: updateActivityPictureUrls.length
              ? updateActivityPictureUrls
              : null,
          },
        },
      );
  ...
  const lookupStarApplication = async () => {
    ...
    try {
      ...
      if (error) {
        ...
        setStarUpdateScore(
          data.requested_contest_score == null
            ? ""
            : String(data.requested_contest_score),
        );
            ...
            <Surface>
              ...
              >
                <Field label="活动 ID">
                  <input
                    className={inputClassName}
                    value={updateActivityId}
                    onChange={(event) =>
                      setUpdateActivityId(event.target.value)
                    }
                    required
                  />
                </Field>
            ...
            <Surface>
              ...
              >
                <Field label="综评活动 ID">
                  <input
                    className={inputClassName}
                    value={generalActivityId}
                    onChange={(event) =>
                      setGeneralActivityId(event.target.value)
                    }
                    required
                  />
                </Field>
                ...
                <div className="flex flex-col sm:flex-row gap-3">
                  <PrimaryButton type="submit" loading={isRecordSubmitting}>
                    创建记录
                  </PrimaryButton>
                  <SecondaryButton
                    type="button"
                    onClick={() => submitRecord("update")}
                ...
                {records.map((record) => (
                  ...
                  >
                    ...
                    <p className="text-sm text-slate-500 mt-1">
                      活动 #{record.activity_id}，申请 {record.requested_score}{" "}
                      分
                    </p>
                  </div>
                ))}
                  ...
                  >
                    ...
                    <button
                      onClick={() => {
                        ...
                        setStarUpdateAttachment(
                          application.contest_attachment || "",
                        );
                        setStarUpdateScore(
                        ...
                        setStarUpdateStatement(
                          application.uniqueness_statement || "",
                        );
                      }}
              ...
              <div className="flex flex-col gap-4">
                <Field label="申请 ID">
                  <input
                    className={inputClassName}
                    value={starLookupId}
                    onChange={(event) => setStarLookupId(event.target.value)}
                  />
                </Field>
              ...
              <form onSubmit={submitStarUpdate} className="flex flex-col gap-4">
                <Field label="更新申请 ID">
                  <input
                    className={inputClassName}
                    value={starUpdateId}
                    onChange={(event) => setStarUpdateId(event.target.value)}
                    required
                  />
                </Field>
                <FileUploadField
