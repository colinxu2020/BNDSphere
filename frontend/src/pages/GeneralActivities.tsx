...
import { Badge, EmptyState, PageHeader, StatusMessage, inputClassName } from '../components/ui/AppPrimitives';
  ...
  return (
    ...
    >
      <PageHeader
        eyebrow="Activities"
        title="综评活动"
        description="浏览校级、大型与社联活动，进入详情查看社团记录。"
      />
      ...
      {isLoading ? (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          {[...Array(4)].map((_, index) => (
            <div key={index} className="animate-pulse bg-white h-40 rounded-[2rem] border border-slate-100" />
          ))}
        </div>
      ) : items.length > 0 ? (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          {items.map((activityItem, index) => (
            ...
            >
              <Link
                ...
                className="group block h-full bg-white p-6 rounded-[2rem] border border-slate-200/60 shadow-sm hover:shadow-[0_8px_30px_-4px_rgba(0,0,0,0.06)] hover:border-primary-100 transition-all duration-300"
              >
                <div className="flex items-start justify-between gap-4">
                  <div className="w-12 h-12 rounded-2xl bg-primary-50 text-primary-600 flex items-center justify-center shrink-0">
                    <Activity size={22} />
                  </div>
                  <ArrowUpRight size={18} className="text-slate-300 group-hover:text-primary-500 group-hover:translate-x-0.5 group-hover:-translate-y-0.5 transition-all" />
                </div>
                <div className="mt-5">
                  ...
                  <p className="text-sm text-slate-500 line-clamp-2 mt-2">{activityItem.description}</p>
